...
/**
 * Customized version of Photo Map, to make it work with UMG together
 * http://www.bizmodules.net/
 *
 * Copyright 2012, Pengcheng Rong
 *
 * Date: Sun Sep 2 2012
 */
...
jQuery(function() {
	if(!umg_photomap_bridge) return;
	
	Map						= function() {		
		// google.maps.Map
		this.map			= null;
		// the DOM element
		this.$mapEl			= jQuery('#map');
		this.$mapEl.data({
		...
		// the map options
		this.mapOptions 	= {
			mapTypeId					: umg_photomap_bridge.MapTypeId,
			mapTypeControl				: true,
	...
	Map.prototype 			= {
		...
		_create				: function() {
			...
			jQuery(window).bind( 'resize.Map', function() {
		...
		_resizeFullScreenMap: function( instance ) {
			...
			var cssStyle	= {};
			cssStyle.width	= jQuery(window).width();
			cssStyle.height	= jQuery(window).height();
			$mpContainer.addClass('mp-container-fullscreen').css( cssStyle );
			instance.$mapEl.css({
				width	: cssStyle.width,
				height 	: cssStyle.height - jQuery.data( Main, 'menu' ).$titleLbl.height()
			});
	...
	Marker.prototype		= {
		_create				: function( data ) {
			...
			this.mapicon		= umg_photomap_bridge.umgpath + 'css/PhotoMap/images/camera.png';
			this.mapiconHover	= umg_photomap_bridge.umgpath + 'css/PhotoMap/images/camera_hover.png';
	...
	Photo.prototype 		= {
		...
		this.umgid	= data.umgid;
			...
			google.maps.event.addListener( this.marker.MapMarker, 'click', function() {
				var gallery 			= jQuery.data( Main, 'gallery' ),
					map					= jQuery.data( Main, 'map' );
			...
			google.maps.event.addListener( this.marker.MapMarker, 'mouseover', function() {
				...
				if( !jQuery.data( Main, 'map' ).isFullscreen() ) {
	...
	Place.prototype 		= {
		_create				: function( data ) {
			...
			google.maps.event.addListener( this.marker.MapMarker, 'click', function() {
				...
				var menu 		= jQuery.data( Main, 'menu' );
		...
		hidePhotos			: function() {
			...
			var map 			= jQuery.data( Main, 'map' ),
				gallery 		= jQuery.data( Main, 'gallery' );
		...
		_showGallery		: function() {
			...
			jQuery.data( Main, 'gallery' ).show( this.photos );
	...
	MapPhotoAlbum.prototype = {
		init				: function() {
			...
			menu.$modeBtn.toggle(
				function( event ) {				
					...
					jQuery(this).addClass('mp-option-normal');
				...
				function( event ) {
					...
					jQuery(this).removeClass('mp-option-normal');
		...
		_getPlaces			: function( callback ) {
			...
			/*jQuery.getJSON('main.class.php', function( data ) {
				...
				jQuery.each( data.places, function( key, placeinfo ) {
			...
			// the album name
			var mapalbumjson = umg_photomap_bridge.Data;//jQuery.parseJSON(umg_photomap_bridge.Data);
			instance.album			= mapalbumjson.name;
			// the album description
			instance.description	= mapalbumjson.description;
			
			jQuery.each( mapalbumjson.places, function( key, placeinfo ) {
				var place = new Place( placeinfo );
				instance.places.push( place );
			});
	...
	Menu.prototype 			= {
		...
		_addTitleControls	: function() {
			...
			this.$backBtn		= jQuery('<a href="#" class="mp-option-back">Back</a>').hide();
			this.$modeBtn		= jQuery('<a href="#" class="mp-option-fullscreen">Fullscreen</a>');
			
			var $options		= jQuery('<div class="mp-options"/>').append( this.$backBtn ).append( this.$modeBtn );
		
			this.$titleLbl		= jQuery('<h2 class="mp-label"></h2>');
			
			$mpContainer.append( 
				jQuery('<div class="mp-options-wrapper"/>').append( $options ).append( this.$titleLbl )
			);
	...
	Gallery					= function() {
		this.$galleryEl	= jQuery('#mp-album-wrapper');
		this._initEvents();
	};
	...
	Gallery.prototype 		= {
		show							: function( photos ) {
			...
			jQuery('#galleryTmpl').tmpl( {tmplPhotosData : tmplPhotosData} )
				...
				$el.addClass('current').siblings('a').removeClass('current');
							...
							jQuery.data( Main, 'map' ).place.clearIcons();
		...
		_startSlider					: function() {
			...
			if( !this.$galleryAlbum ) {
				...
				jQuery('#galleryAlbumTmpl').tmpl( {tmplPhotoData : data} )
									  .appendTo( $mpContainer );
									  
				this.$galleryAlbum	= jQuery('#mp-album-overlay');
			...
			jQuery.data( Main, 'menu' ).setTitle( this.currentPhoto.description );
			if (typeof(UPG_onPhotoLoad) == "function")
			{
				UPG_onPhotoLoad(this.currentPhoto.umgid);
			}
		...
		_navigateSlider					: function( instance, dir ) {
			...
			jQuery('<img/>').load(function() {
				if (typeof(UPG_onPhotoLoad) == "function")
				{
					UPG_onPhotoLoad(instance.currentPhoto.umgid);
				}
		
				instance.$loading.hide();
				instance.$currentImage.attr( 'src', instance.currentPhoto.source );
			jQuery.data( Main, 'menu' ).setTitle( instance.currentPhoto.description );
		...
		zoom							: function() {
			...
			if( !this.$galleryFullscreen ) {
				...
				jQuery('#galleryFullscreenTmpl').tmpl( {tmplPhotoData : data} )
										   .appendTo( $mpContainer );
				
				this.$galleryFullscreen				= jQuery('#mp-image-overlay');
		...
		_resizeImage					: function( $image ) {
			...
			var widthMargin		= 0,
				...
				windowH      	= jQuery(window).height() - heightMargin,
				windowW      	= jQuery(window).width() - widthMargin,
				theImage     	= new Image();
	...
	var $mpContainer		= jQuery('#mp-container'),
		Main	= (function() {
			
		var init	= function() {
			
				// instance of Map
			jQuery.data( Main, 'map', new Map() );
				// instance of Menu
				jQuery.data( Main, 'menu', new Menu() );
				// instance of Gallery
				jQuery.data( Main, 'gallery', new Gallery() );
