...
{
    window.addEventListener("load", drag_initialize, false);
}
...
{
	window.onload = function(){drag_initialize();}
}
...
function drag_initialize (){
	...
	for (var i=0; i<divs.length; i++) 
		...
		{
			//if this element have a child with the name of "hotspot", then drag this child, othewise drag this element
			var foundHotspot = false;
			var childs = divs[i].getElementsByTagName('div');
			for (var j=0; j<childs.length; j++) 
			{
				if (childs[j].id == 'hotspot')
				{
					foundHotspot = true;
					childs[j].onmousedown = handler_onmousedown;
				}
			}
			if (!foundHotspot)
			{
...
function handler_onmousedown(e){
	if ((typeof dragConfirm) != "undefined")
	{
		if (!dragConfirm())
		{
			return;
...
function handler_onmouseup(e){
	...
	// switch source and destination content
		...
		if (destination_elements_length) myhandler_switched();
		else myhandler_dropped();
	}
...
function myhandler_moved()     {}
function myhandler_notmoved()  {}
function myhandler_dropped()   {}
function myhandler_switched()  {}
function myhandler_cloned()    {}
function myhandler_notcloned() {}
function myhandler_deleted()   {}
function myhandler_undeleted() {}
