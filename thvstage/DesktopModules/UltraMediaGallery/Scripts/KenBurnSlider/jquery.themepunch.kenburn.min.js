(function($,undefined){$.fn.extend({kenburn:function(options){var defaults={responsive:"on",noresponsiveWidth:600,noresponsiveHeight:300,fixHeight:false,thumbWidth:50,thumbHeight:50,thumbAmount:6,thumbStyle:"bullet",thumbXOffset:0,thumbYOffsetm:0,bulletXOffset:0,bulletYOffset:0,shadow:'true',timer:2000,touchenabled:"on",pauseOnRollOverThumbs:'off',pauseOnRollOverMain:'on',preloadedSlides:50,googleFonts:'PT+Sans+Narrow:400,700',googleFontJS:'http://ajax.googleapis.com/ajax/libs/webfont/1/webfont.js',debug:"no",rescaleBGColor:"#333"};options=$.extend({},$.fn.kenburn.defaults,options);WebFontConfig={google:{families:[options.googleFonts]},active:function(){jQuery('body').data('googlefonts','loaded');},inactive:function(){jQuery('body').data('googlefonts','loaded');}};return this.each(function(){var opt=options;if(opt.debug=="yes"||opt.debug=="on")
$('body').append('<div class="testinfo" style="z-index:99999;position:fixed;background:#000;color:#fff;width:700px;height:200px;top:10px;left:10px;"></div>');if(opt.googleFonts!=undefined&&opt.googleFonts.length>0){var wf=document.createElement('script');wf.src=opt.googleFontJS;wf.type='text/javascript';wf.async='true';var s=document.getElementsByTagName('script')[0];s.parentNode.insertBefore(wf,s);jQuery('body').data('googlefonts','wait');}else{jQuery('body').data('googlefonts','loaded');}
opt.savedTimer=opt.timer;var top_container=$(this);if(top_container.attr('id')==undefined)top_container.attr('id',Math.round(Math.random()*100000));var minW=top_container.css('minWidth');var maxW=top_container.css('maxWidth');opt.whproportion=parseInt(top_container.css('height'),0)/parseInt(top_container.css('maxWidth'),0);opt.width=top_container.width()-10;opt.height=top_container.height()-10;if(!opt.fixHeight||!opt.fixHeight=="true"){opt.height=opt.width*opt.whproportion;}
opt.originalwidth=parseInt(top_container.css('maxWidth'),0);opt.originalheight=parseInt(top_container.css('height'),0);if(!opt.fixHeight||!opt.fixHeight=="true"){opt._propw=opt.width/opt.originalwidth;opt._proph=opt.height/opt.originalheight;}else{opt._propw=1;opt._proph=1;}
top_container.height(opt.height+10);opt.oldid="responsive"+top_container.attr('id');top_container.wrap('<div id="'+opt.oldid+'" class="responsiveholder"></div>');opt.fullHTML=$('body').find('#'+opt.oldid).html();var responsive=true;if(minW==maxW||maxW=="none")responsive=false;if(opt.responsive=="off"){responsive=false;opt.width=opt.noResponsiveWidth;opt.height=opt.noResponsiveHeight;}
if(opt.width<500&&responsive&&!opt.fixHeight){opt._propw=opt._propw*1.7;opt._proph=opt._proph*1.7;}
startToPrepare(top_container,opt);opt.videoWasOn=-1;if(responsive)
$(window).resize(function(){if(top_container.find('.videoon').length>0&&opt.videoWasOn==-1){opt.videoWasOn=1}else{if(opt.videoWasOn==-1)opt.videoWasOn=0;}
clearInterval(opt.aktKenInterval);clearInterval(opt.timerinterval);clearInterval(opt.waitForWF);clearTimeout(opt.resetAll);opt.width=top_container.width()-10;if(!opt.fixHeight||!opt.fixHeight=="true"){opt.height=opt.width*opt.whproportion;opt._propw=opt.width/opt.originalwidth;opt._proph=opt.height/opt.originalheight;}
if(opt.width<500&&responsive&&!opt.fixHeight){opt._propw=opt._propw*1.7;opt._proph=opt._proph*1.7;}
opt.preloadedSlides=200;top_container.empty();top_container.remove();$('body').find('#'+opt.oldid).append(opt.fullHTML);top_container=$('body').find('#'+opt.oldid).find('div:first');top_container.height(opt.height+10);opt.resetAll=setTimeout(function(){startToPrepare(top_container,opt);},200);});})}})
function startToPrepare(top_container,opt){if(opt.debug==="on")
$('body').append('<div class="khinfo" style="background:#fff;color:#000;width:300px;height:250px;position:fixed;left:10px;top:10px;"></div>');top_container.append('<div class="kenburn-preloader"></div>');$('body').find('.khinfo').html('Start Slider');prepareSlidesContainer(top_container,opt);$('body').find('.khinfo').html('Prepared Container');prepareSlides(top_container,opt);$('body').find('.khinfo').html('Prepared Preloaded Slides');$('body').find('.khinfo').html('Waiting for Images...');opt.loadedImages=0;top_container.waitForImages(function(){$('body').find('.khinfo').html('Preloaded Images has been loaded');opt.waitForWF=setInterval(function(){$('body').find('.khinfo').html('Waiting for Google Fonts');if($('body').data('googlefonts')!=undefined&&$('body').data('googlefonts')=="loaded"){opt.lastThumb=opt.currentThumb;if(opt.thumbStyle=="image"||opt.thumbStyle=="both"||opt.thumbStyle=="thumb")
...
opt.padtop=parseInt(bg.css('paddingTop'),0);if(bg.css('padingLeft')!=undefined&&bg.css('padingLeft')!=null)
if(bg.css('paddingLeft').length>0)
opt.padleft=parseInt(bg.css('paddingLeft'),0);if(bg.css('padingRight')!=undefined&&bg.css('padingRight')!=null)
if(bg.css('paddingRight').length>0)
opt.padright=parseInt(bg.css('paddingRight'),0);if(bg.css('padingBottom')!=undefined&&bg.css('padingBottom')!=null)
if(bg.css('paddingBottom').length>0)
opt.padbottom=parseInt(bg.css('paddingBottom'),0);if(bg.css('borderTop')!=undefined&&bg.css('borderTop')!=null)
if(bg.css('borderTop').length>0)
opt.botop=parseInt(bg.css('borderTop'),0);if(bg.css('borderLeft')!=undefined&&bg.css('borderLeft')!=null)
if(bg.css('borderLeft').length>0)
opt.boleft=parseInt(bg.css('borderLeft'),0);if(bg.css('borderRight')!=undefined&&bg.css('borderRight')!=null)
if(bg.css('borderRight').length>0)
opt.boright=parseInt(bg.css('borderRight'),0);if(bg.css('borderBottom')!=undefined&&bg.css('borderBottom')!=null)
if(bg.css('borderBottom').length>0)
opt.bobottom=parseInt(bg.css('borderBottom'),0);}else{if(bg.css('paddingTop').length>0)
opt.padtop=parseInt(bg.css('paddingTop'),0);if(bg.css('paddingLeft').length>0)
opt.padleft=parseInt(bg.css('paddingLeft'),0);if(bg.css('paddingRight').length>0)
opt.padright=parseInt(bg.css('paddingRight'),0);if(bg.css('paddingBottom').length>0)
opt.padbottom=parseInt(bg.css('paddingBottom'),0);if(bg.css('borderTop').length>0)
opt.botop=parseInt(bg.css('borderTop'),0);if(bg.css('borderLeft').length>0)
opt.boleft=parseInt(bg.css('borderLeft'),0);if(bg.css('borderRight').length>0)
opt.boright=parseInt(bg.css('borderRight'),0);if(bg.css('borderBottom').length>0)
opt.bobottom=parseInt(bg.css('borderBottom'),0);}}
function createThumbnails(top,opt){var maxitem=top.find('ul >li').length;var maxwidth=opt.thumbAmount*opt.thumbWidth;opt.thumbmaxwidth=maxwidth;var maxheight=opt.thumbHeight;var bgwidth=maxwidth;var bgheight=maxheight;var full=opt.width+opt.padleft+opt.padright;var centerl=Math.round(full/2-bgwidth/2);var max=(maxitem*opt.thumbWidth);top.append('<div class="kenburn_thumb_container" style="position:absolute;left:'+centerl+'px;top:'+(1+opt.height+opt.padtop+opt.padbottom)+'px;width:'+(bgwidth+2)+'px;height:'+(bgheight+2)+'px;"></div>');var thc=top.find('.kenburn_thumb_container');if(opt.hideThumbs=="on")thc.css({'opacity':0});if(opt.thumbAmount==0)thc.css({'visibility':'hidden'});thc.append('<div class="kenburn_thumb_container_bg" style="position:absolute;left:1px;top:0px;width:'+(bgwidth)+'px;height:'+(bgheight)+'px"></div>');thc.append('<div id="thumbmask" style="overflow:hidden;position:absolute;top:0px;left:1px; width:'+maxwidth+'px; height:'+opt.thumbHeight+'px;overflow:hidden;"></div>');var thma=thc.find('#thumbmask');thma.append('<div class="thumbsslide" style="width:'+max+'px"></div>');var thbg=thma.find('.thumbsslide');thc.hover(function(){$(this).addClass('overme');thc.animate({'opacity':1},{duration:300,queue:false});},function(){$(this).removeClass('overme');var sm=top.find('.slide_mainmask');setTimeout(function(){if(opt.hideThumbs=="on"&&!sm.hasClass('overon')&&!$('body').hasClass('tp_showthumbsalways')){thc.animate({'opacity':0},{duration:300,queue:false});}},10);});top.find('ul >li').each(function(i){var $this=$(this);var img=$this.find('img:first');var src=img.data('thumb');var isvideo=$this.find('.slide_container').data('video')==1;var thumb=$('<div class="kenburn-thumbs" id="thumb'+i+'" style="cursor:pointer;position:absolute;top:0px;left:'+(i*opt.thumbWidth)+'px;width:'+opt.thumbWidth+'px;height:'+opt.thumbHeight+'px;background:url('+src+');"></div>');thumb.data('id',i);var overlay=$('<div class="overlay"></div>');thumb.append(overlay);if(i==maxitem)thumb.css({'margin-right':'0px'});thbg.append(thumb);if(i==opt.lastThumb){thumb.addClass("selected");thumb.find('.overlay').css({'opacity':0});}else{thumb.find('.overlay').css({'opacity':0.75});thumb.removeClass("selecte");}
if(isvideo&&opt.thumbVideoIcon=="on"){var new_video=$('<div class="video"></div>');thumb.append(new_video);thumb.find('.video').css({'position':'absolute','top':opt.thumbHeight/2+'px','left':opt.thumbWidth/2+'px','z-index':'1000'});}
var thumbnail=thbg.find('#thumb'+i);thumbnail.hover(function(){var thumb=$(this);thumb.stop();thumb.find('.overlay').animate({'opacity':0},{duration:300,queue:false});},function(){var thumb=$(this);thumb.stop();if(!thumb.hasClass('selected'))
thumb.find('.overlay').animate({'opacity':0.75},{duration:300,queue:false});});thumbnail.click(function(){var $this=$(this);if(($this.position().left+opt.thumbWidth)==opt.thumbmaxwidth&&$this.index()!=maxitem-1){$this.parent().find('.kenburn-thumbs').each(function(){var thumb=$(this);thumb.cssAnimate({'left':(thumb.position().left-opt.thumbWidth)+"px"},{duration:300,queue:false});});}
if(($this.position().left)==0&&$this.index()>0){$this.parent().find('.kenburn-thumbs').each(function(){var thumb=$(this);thumb.cssAnimate({'left':(thumb.position().left+opt.thumbWidth)+"px"},{duration:300,queue:false});});}
if(!$this.hasClass("selected")){var newslide=top.find('ul li:eq('+$this.data('id')+')');swapBanner(top.data('currentSlide'),newslide,top,opt);}});});opt.thumbVertical="bottom";if(opt.thumbVertical=="bottom"){thc.css({'z-index':1000,'top':opt.thumbYOffset+(opt.height-opt.thumbHeight)+"px"});}else{if(opt.thumbVertical=="center"){thc.css({'z-index':1000,'top':opt.thumbYOffset+(opt.height/2-opt.thumbHeight/2)+"px"});}else{thc.css({'z-index':1000,'top':(opt.thumbYOffset)+"px"});}}
if(opt.thumbHorizontal=="left"){thc.css({'left':opt.thumbXOffset+"px"});}else{if(opt.thumbHorizontal=="right"){thc.css({'left':opt.thumbXOffset+(opt.width-maxwidth)+"px"});}else{thc.css({'left':opt.thumbXOffset+thc.position().left+"px"});}}
if(opt.hideThumbs=="on"){thc.removeClass('overme');var sm=top.find('.slide_mainmask');setTimeout(function(){if(opt.hideThumbs=="on"&&!sm.hasClass('overon')&&!$('body').hasClass('tp_showthumbsalways')){thc.animate({'opacity':0},{duration:0,queue:false});}},10);}}
...
setTimeout(function(){item.find('.kenburn-preloader').remove();},300);if(opt.lastThumb==undefined)opt.lastThumb=0;var first_slide=item.find('ul li:eq('+opt.lastThumb+')');swapBanner(first_slide,first_slide,item,opt);startParallax(item,opt);opt.loaded=1;if(opt.videoWasOn==1)
item.parent().find('.kenburn-video-button').click();opt.videoWasOn=-1;}
function startParallax(slidertop,opt){var top=slidertop.find('.slide_mainmask');top.data('maxwidth',opt.width+opt.padleft+opt.padright);top.data('maxheight',opt.height+opt.padtop+opt.padbottom);top.data('pdistance',opt.parallaxX);top.data('pdistancey',opt.parallaxY);top.data('cdistance',opt.captionParallaxX);top.data('cdistancey',opt.captionParallaxY);top.data('opt',opt);var slide=top.parent().data('currentSlide');var par=top.find('.parallax_container');var layers=slide.find('.layer_container');top.mouseenter(function(e){var $this=$(this);var thma=$this.parent().find('.kenburn_thumb_container #thumbmask');var slide=$this.parent().data('currentSlide');var par=slide.find('.parallax_container');var layers=slide.find('.layer_container');$this.addClass('overon');clearTimeout(opt.hideThumb);slidertop.find('.kenburn_thumb_container').animate({'opacity':1},{duration:300,queue:false});});top.mouseleave(function(e){var $this=$(this);var slide=$this.parent().data('currentSlide');var par=slide.find('.parallax_container');var layers=slide.find('.layer_container');$this.removeClass("overme");$this.removeClass('overon');var thc=slidertop.find('.kenburn_thumb_container');setTimeout(function(){if(opt.hideThumbs=="on"&&!thc.hasClass('overme')&&!$('body').hasClass('tp_showthumbsalways')){slidertop.find('.kenburn_thumb_container').animate({'opacity':0},{duration:300,queue:false});}},10);});top.mousemove(function(e){var $this=$(this);if(opt.pauseOnRollOverMain!="off")
$this.addClass("overme");var thma=$this.parent().find('.kenburn_thumb_container #thumbmask');slidertop.find('.kenburn_thumb_container').css({'display':'block'});if(!thma.hasClass('overme')&&!$this.hasClass('overon')){var slide=$this.parent().data('currentSlide');var par=slide.find('.parallax_container');var layers=slide.find('.layer_container');}else{setTimeout(function(){$this.removeClass('overon')},100);}});}
...
function swapBanner(item,newitem,slider_container,opt){clearInterval(opt.aktKenInterval);var trans=false;slider_container.find('ul >li').each(function(i){var $this=$(this);clearInterval($this.data('interval'));if($this.index()!=item.index()&&$this.index()!=newitem.index()){$this.css({'display':'none','position':'absolute','left':'0px','z-index':'994'});}});var video=slider_container.find('.slide_mainmask .video_kenburn');setTimeout(function(){video.remove()},600);var slidemask=slider_container.find('.slide_mainmask');slidemask.removeClass("videoon");item.css({'position':'absolute','top':'0px','left':'0px','z-index':'900'});newitem.css({'position':'absolute','top':'0px','left':'0px','z-index':'1000'});newitem.css({'display':'block'});var sour=newitem.find('img:first');var sourbw=newitem.find('.bw-main-image');var kenburn=true;if(newitem.data('kenburn')=="off"||(navigator.userAgent.match(/Android/i)))
{hasCanvas=false;kenburn=false;newitem.data('startalign','center,center');newitem.data('endalign','center,center');newitem.data('zoom','none');newitem.data('zoomfact',0);}
if(newitem.data('ww')==undefined){var oldW=newitem.find('img').width()
var oldH=newitem.find('img').height()
if(oldW!=0){if(sour.width()>0&&sour.width()<opt.width){var factor=opt.width/oldW;oldW=oldW*factor;oldH=oldH*factor;}
...
clearInterval(item.data('interval'));sour.parent().find('.canvas-now').remove();sour.parent().find('.canvas-now-bw').remove();if(kenburn){var hasCanvas=isCanvasSupported();var isIEunder9=$.browser.msie&&$.browser.version<9;if(isIEunder9)hasCanvas=false;}
var hascaption=true;if(newitem.find('.creative_layer div').length>0){var clw=newitem.find('.creative_layer div').outerWidth();var clh=newitem.find('.creative_layer div').outerHeight();var clt=newitem.find('.creative_layer div').position().top;var cll=newitem.find('.creative_layer div').position().left;}else{hascaption=false;var clw=0;var clh=0;var clt=0;var cll=0;sourbw.remove();}
if(hasCanvas){sour.parent().append('<div style="position:absolute;z-index:1" class="canvas-now"><canvas class="canvas-now-canvas" width="'+oldW+'" height="'+oldH+'"></canvas></div>');sour.css({'display':'none'});var canvas=sour.parent().find('.canvas-now-canvas')[0];var context=canvas.getContext('2d');if(sourbw.length>0){sour.parent().append('<div style="position:absolute;z-index:20" class="canvas-now-bw"><canvas class="canvas-now-canvas-bw" width="'+oldW+'" height="'+oldH+'"></canvas></div>');sourbw.css({'display':'none'});var canvasbw=sour.parent().find('.canvas-now-canvas-bw')[0];sour.parent().find('.canvas-now-bw').wrap('<div class="blurwrap" style="width:'+clw+'px;height:'+clh+'px;position:absolute;top:'+clt+'px;left:'+cll+'px;overflow:hidden"></div>');var contextbw=canvasbw.getContext('2d');}}else{if(sourbw.parent().hasClass("blurwrap")){}else{sourbw.wrap('<div class="blurwrap" style="width:'+clw+'px;height:'+clh+'px;position:absolute;top:'+clt+'px;left:'+cll+'px;overflow:hidden;"></div>');}}
sour.css({'-webkit-transform':'translateZ(0)'});sourbw.css({'-webkit-transform':'translateZ(0)'});var time=newitem.data('panduration');var scalerX=0;var scalerY=0;var newW=oldW;var newH=oldH;var zoomfact=newitem.data('zoomfact')
...
sour.parent().find('.canvas-now-bw').css({'opacity':0});if(hasCanvas){context.drawImage(sour[0],newL,newT,newW,newH);if(sourbw.length>0&&hascaption){contextbw.drawImage(sourbw[0],newL,newT,newW,newH);setTimeout(function(){sour.parent().find('.canvas-now-bw').cssAnimate({'opacity':'1.0'},{duration:1000,queue:false});},1200);}}
sour.cssStop(true,true);sourbw.cssStop(true,true);if(kenburn){sour.css({'position':'absolute','left':newL+"px",'top':newT+"px",'width':newW+"px",'height':newH+"px",'opacity':1.0});sourbw.css({'position':'absolute','left':newL+"px",'top':newT+"px",'width':newW+"px",'height':newH+"px",'opacity':1.0});var oldL=newL;var oldT=newT;var oldWW=newW;step=0;}else{var wFF=opt.width/newW;var hFF=opt.height/newH;if(wFF>hFF){newW=opt.width;newH=newH*wFF;}else{newW=newW*hFF;newH=opt.height;}
sour.css({'position':'absolute','left':newL+"px",'top':newT+"px",'width':newW+"px",'height':newH+"px",'opacity':1.0});sourbw.css({'position':'absolute','left':(newL-cll)+"px",'top':(newT-clt)+"px",'width':newW+"px",'height':newH+"px",'opacity':0.0});setTimeout(function(){sourbw.animate({'opacity':'1.0'},{duration:1000,queue:false});},1200);}
if(kenburn){opt.aktKenInterval=setInterval(function(){$('body').find('.testinfo').html(opt.aktKenInterval+"  "+Math.floor(Math.random()*10000));if(!slider_container.parent().parent().find('.kenburn_thumb_container #thumbmask').hasClass('overme')&&!slider_container.find('.slide_mainmask').hasClass('overme')&&!slider_container.find('.slide_mainmask').hasClass('videoon')){newW=newW+scalerX;newH=newH+scalerY;newL=newL+movX;newT=newT+movY;if(newL>0)newL=0;if(newT>0)newT=0;if(newL<(opt.width-newW))newL=opt.width-newW;if(newT<(opt.height-newH))newT=opt.height-newH;if(hasCanvas){context.drawImage(sour[0],newL,newT,newW,newH);if(sourbw.length>0)contextbw.drawImage(sourbw[0],(newL-cll),(newT-clt),newW,newH);}else{var s=newW/oldWW;var p1=newL-oldL;var p2=newT-oldT;var p3=newL-oldL-cll;var p4=newT-oldT-clt;if(jQuery.browser.msie){sour.css({'filter':'progid:DXImageTransform.Microsoft.Matrix(FilterType="bilinear",M11='+s+',M12=0,M21=0,M22='+s+',Dx='+p1+',Dy='+p2+')'});sour.css({'-ms-filter':'progid:DXImageTransform.Microsoft.Matrix(FilterType="bilinear",M11='+s+',M12=0,M21=0,M22='+s+',Dx='+p1+',Dy='+p2+')'});sourbw.remove();}else{sour.cssAnimate({'left':newL+"px",'top':newT+"px",'width':newW+"px",'height':newH+"px"},{duration:38,easing:'linear',queue:false});if(sourbw.length>0){sourbw.cssAnimate({'left':newL+"px",'top':newT+"px",'width':newW+"px",'height':newH+"px"},{duration:38,easing:'linear',queue:false});}}}}},40);}
var is_chrome=/chrome/.test(navigator.userAgent.toLowerCase());if(is_chrome&&opt.repairChromeBug=="on"){newitem.data('transition','slide');}
if(newitem.data('transition')=="slide"){if(trans==false){var left=true;if(item.index()>newitem.index())left=false;if(left){video.animate({'left':(0-opt.width)+'px'},{duration:600,queue:false});newitem.find('.slide_container').stop(true,true);newitem.find('.slide_container').css({'opacity':'1.0','left':opt.width+"px"});item.find('.slide_container').animate({'left':0-opt.width+'px'},{duration:600,queue:false});newitem.find('.slide_container').animate({'left':'0px','top':'0px','opacity':'1.0'},{duration:600,queue:false});}else{video.animate({'left':(opt.width)+'px'},{duration:600,queue:false});newitem.find('.slide_container').stop(true,true);newitem.find('.slide_container').css({'opacity':'1.0','position':'absolute','top':'0px','left':0-opt.width+'px'});item.find('.slide_container').animate({'left':opt.width+'px'},{duration:600,queue:false});newitem.find('.slide_container').animate({'left':'0px','top':'0px','opacity':'1.0'},{duration:600,queue:false});}}}else{item.find('.slide_container').stop(true,true);item.find('.slide_container').animate({'opacity':'0'},{duration:600,queue:false});video.animate({'opacity':'0.0'},{duration:600,queue:false});newitem.find('.slide_container').stop(true,true);newitem.find('.slide_container').css({'opacity':'0.0','left':'0px','top':'0px'});newitem.find('.slide_container').animate({'opacity':'1.0'},{duration:600,queue:false});}
if(slider_container.find('.kenburn_thumb_container').length>0){var thumb=slider_container.find('.kenburn_thumb_container #thumbmask .thumbsslide #thumb'+newitem.index());slider_container.find('.kenburn_thumb_container #thumbmask .thumbsslide #thumb'+item.index()).each(function(i){var $this=$(this);if($this.attr('id')!="thumb"+newitem.index()){$this.removeClass('selected');$this.stop();$this.find('.overlay').animate({'opacity':0.75},{duration:300,queue:false});}});thumb.addClass('selected');thumb.find('.overlay').animate({'opacity':0},{duration:300,queue:false});slider_container.data('currentThumb',thumb);opt.currentThumb=thumb.index();if((thumb.position().left+opt.thumbWidth)==opt.thumbmaxwidth&&thumb.index()!=opt.maxitem-1){thumb.parent().find('.kenburn-thumbs').each(function(){var thumbs=$(this);thumbs.cssAnimate({'left':(thumbs.position().left-opt.thumbWidth)+"px"},{duration:300,queue:false});});}
if((thumb.position().left)==0&&thumb.index()>0){thumb.parent().find('.kenburn-thumbs').each(function(){var thumbs=$(this);thumbs.cssAnimate({'left':(thumbs.position().left+opt.thumbWidth)+"px"},{duration:300,queue:false});});}
if(opt.thumbWidth)
if(thumb.index()==0){thumb.parent().find('.kenburn-thumbs').each(function(){var thumbs=$(this);thumbs.cssAnimate({'left':0+(thumbs.index()*opt.thumbWidth)+"px"},{duration:300,queue:false});});}}
if(slider_container.find('.minithumb').length>0){var thumb=slider_container.find('#minithumb'+newitem.index());slider_container.find('.minithumb').removeClass('selected');thumb.addClass('selected');if(opt.thumbStyle!="both")slider_container.data('currentThumb',thumb);opt.currentThumb=thumb.index();}
slider_container.data('currentSlide',newitem);textanim(newitem,100,slider_container);opt.cd=0;}
function isCanvasSupported(){var elem=document.createElement('canvas');return!!(elem.getContext&&elem.getContext('2d'));}
function startTimer(top,opt){opt.cd=0;if($.browser.msie)
top.find('.kenburn_thumb_container #thumbmask .thumbsslide').cssAnimate({'left':'0px'},{duration:300,queue:false});else
top.find('.kenburn_thumb_container #thumbmask .thumbsslide').animate({'left':'0px'},{duration:300,queue:false});var tmask=top.find('.kenburn_thumb_container #thumbmask');var tslide=tmask.find('.thumbsslide');var slidemask=top.find('.slide_mainmask');var slidemask=top.find('.slide_mainmask');opt.timerinterval=setInterval(function(){if(opt.loaded==1){var newitem=top.data('currentSlide');var thumb=top.data('currentThumb');if(!slidemask.hasClass('overme')&&!slidemask.hasClass('videoon')){opt.cd=opt.cd+1;if(opt.cd==opt.timer){opt.cd=0;if(newitem!=undefined){if(newitem.index()<opt.maxitem-1){var next=top.find('ul li:eq('+(newitem.index()+1)+')');swapBanner(newitem,next,top,opt);if(opt.thumbStyle!="none"){var minus=0-parseInt(thumb.parent().css('left'),0);if(tslide!=null&&tslide!=undefined){tslide.css({'position':'absolute'});if(Math.abs(minus)<=top.data('thumbnailmaxdif')){if($.browser.msie)
