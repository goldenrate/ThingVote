...
namespace DotNetNuke.Modules.SocialPolls
{
	public class FilesStatus {
		public const string HandlerPath = "/DesktopModules/SocialPolls/";
        ...
        public Status delete_status { get; set; }
        public Status tnails_delete_status { get; set; }
        ...
        public FilesStatus(string fileName, int fileLength, string relUrl)
        {
            SetValues(fileName, fileLength, relUrl);
        }
		...
		private void SetValues (string fileName, int fileLength, string relativeUrl) {
			name = fileName;
			type = "image/png";
			size = fileLength;
			progress = "1.0";
            url = relativeUrl;
            thumbnail_url = HandlerPath + "Thumbnail.ashx?f=" + 
                fileName + "&tnmaxwidth={1}&tnmaxheight={2}&&ownerId={3}"; 
			delete_url = HandlerPath + "FileTransferHandler.ashx?f=" + fileName  ;
			delete_type = "DELETE";
		}
	}
